...
import controllersTest.addOrganizationControllerTest.{documentTypeSeq, getOrganizationID}
import controllersTest.addZoneControllerTest.{documentTypeSeq, getZoneID}
import controllersTest.changeBuyerBidControllerTest.getAddressFromAccountID
import controllersTest.issueAssetControllerTest.{allIssueAssetDocumentType, getAssetPriceForIssueAsset, getAssetQuantityForIssueAsset, getAssetTypeForIssueAsset, getDocumentHashForIssueAsset, getQuantityUnitForIssueAsset, issueAssetDocumentType}
import controllersTest.issueFiatControllerTest.{getTransactionAmountForIssueFiat, getTransactionIDForIssueFiat}
import controllersTest.setACLControllerTest.documentTypeSeq
...
class RampTest2 extends Simulation {

  val scenarioBuilderZoneOrganization2=scenario("Zone/Organization Flow2")
    .exec{session=> session.set(Test.TEST_USERNAME,"ZONE57005").set(Test.TEST_PASSWORD,"ZONE57005")}
    .exec(signUpControllerTest.signUpWithoutFeederScenario)
    .exec(loginControllerTest.loginWithoutSignUpScenario)
    .exec(sendCoinControllerTest.requestFaucetScenario)
    .exec(sendCoinControllerTest.approveFaucetRequestScenario)
    .exec{session=> session.set(Test.TEST_ZONE_ACCOUNT_ID,Test.TEST_ZONE_USERNAME_UNIQUE)}
    .exec(addZoneControllerTest.addZoneRequestScenario)
    .exec(addZoneControllerTest.verifyZoneScenario)
    .exec{session=> session.set(Test.TEST_USERNAME,"ORG57003").set(Test.TEST_PASSWORD,"ORG57003")}
    .exec(signUpControllerTest.signUpWithoutFeederScenario)
    .exec(loginControllerTest.loginWithoutSignUpScenario)
    .exec(sendCoinControllerTest.requestFaucetScenario)
    .exec(sendCoinControllerTest.approveFaucetRequestScenario)
    ...
    .exec{session=> session.set(Test.TEST_USERNAME,Test.TEST_ZONE_USERNAME_UNIQUE).set(Test.TEST_PASSWORD,Test.TEST_ZONE_PASSWORD_UNIQUE)}
    .exec(addOrganizationControllerTest.updateAndVerifyOrganizationKYCStatusScenario)
  ...
  val tradingScenario=scenario("tradingScenario")
    ...
    .doIf(session=> session(Test.TEST_ZONE_ID).as[String] == "0") {
      asLongAsDuring(session=> session(Test.TEST_ZONE_ID).as[String] == "0",Duration.create(30,"seconds")) {
        pause(1)
          .exec { session => session.set(Test.TEST_ZONE_ID, getOrganizationID(Test.TEST_ZONE_USERNAME_UNIQUE))}
      }
    }
  ...
  setUp(
    tradingScenario.inject(atOnceUsers(1))
  )
    .maxDuration(800)
  ...
  val masterZoneSignUp: ScenarioBuilder = scenario("masterZoneSignUp")
    ...
    .exec(http("ZoneSignUp_POST")
      ...
      .formParamMap(Map(
        ...
        Form.CONFIRM_PASSWORD -> Test.TEST_ZONE_PASSWORD_UNIQUE,
...
object loginMainApproveFaucetZone2 {
  ...
  val masterLoginMainAndApproveFaucetRequestZone: ScenarioBuilder = scenario("masterLoginMainAndApproveFaucetRequestZone")
    ...
    .exec(http("ApproveFaucetRequestZone_GET")
      .get("/master/approveFaucetRequests?requestID=" + "${%s}".format(Test.TEST_REQUEST_ID) + "&accountID=" + Test.TEST_ZONE_USERNAME_UNIQUE)
      .check(css("[name=%s]".format(Form.CSRF_TOKEN), "value").saveAs(Form.CSRF_TOKEN)))
    .pause(2)
    .exec(http("ApproveFaucetRequestZone_POST")
      ...
      .formParamMap(Map(
        ...
        Form.CSRF_TOKEN -> "${%s}".format(Form.CSRF_TOKEN))))
    .pause(5)
    .exec{session => session.set(Test.USER_TYPE, sendCoinControllerTest.getUserType(Test.TEST_ZONE_USERNAME_UNIQUE)) }
  ...
  val masterOrganizationSignUp: ScenarioBuilder = scenario("masterOrganizationSignUp")
    ...
    .exec(http("OrganizationSignUp_POST")
      ...
      .formParamMap(Map(
        ...
        Form.CONFIRM_PASSWORD -> Test.TEST_ORGANIZATION_PASSWORD_UNIQUE,
        Form.CSRF_TOKEN -> "${%s}".format(Form.CSRF_TOKEN))))
    .pause(3)
  ...
  val masterLoginMainAndApproveFaucetRequestOrganization: ScenarioBuilder = scenario("masterLoginMainAndApproveFaucetRequestOrganization")
    ...
    .exec(http("ApproveFaucetRequest_POST")
      ...
      .formParamMap(Map(
        ...
        Form.CSRF_TOKEN -> "${%s}".format(Form.CSRF_TOKEN))))
    .pause(2)
    .exec{session => session.set(Test.USER_TYPE, sendCoinControllerTest.getUserType(Test.TEST_ORGANIZATION_USERNAME_UNIQUE)) }
    .doIf(session=> session(Test.USER_TYPE).as[String] != "USER") {
      asLongAsDuring(session=> session(Test.USER_TYPE).as[String] != "USER",,Duration.create(80,"seconds")) {
        pause(1)
          .exec { session =>session.set(Test.USER_TYPE, sendCoinControllerTest.getUserType(Test.TEST_ORGANIZATION_USERNAME_UNIQUE)) }

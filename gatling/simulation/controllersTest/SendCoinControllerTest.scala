...
class SendCoinControllerTest extends Simulation {
  ...
  object sendCoinControllerTest{
    ...
    val sendCoinScenario: ScenarioBuilder = scenario("SendCoin")
      ...
      .exec { session => {session.set(Test.TEST_TO, sqlQuery(session(Test.TEST_USERNAME).as[String]).apply().next()("accountAddress"));session}}//toFeed
    ...
    val sendCoinMainScenario: ScenarioBuilder = scenario("SendCoinMain")
      ...
      .feed(PasswordFeeder.passwordFeed)
      .feed(GasFeeder.gasFeed)
      .exec { session => {session.set(Test.TEST_TO, sqlQuery(session(Test.TEST_MAIN_USERNAME).as[String]).apply().next()("accountAddress"));session}}//toFeed
    ...
    val blockchainSendCoinMainScenario: ScenarioBuilder = scenario("BlockchainSendCoinMain")
      ...
      .feed(GasFeeder.gasFeed)
      .exec {session =>
        println(sqlQuery(session(Test.TEST_USERNAME).as[String]).apply().next()("accountAddress"))
        session.setAll(
          Test.TEST_TO -> sqlQuery(session(Test.TEST_USERNAME).as[String]).apply().next()("accountAddress"),
  ...
  */
  ...
  .exec(http("ApproveFaucetRequest_GET")
    .get(routes.SendCoinController.approveFaucetRequestsForm().url)
    .check(css("[name=%s]".format(Form.CSRF_TOKEN), "value").saveAs(Form.CSRF_TOKEN)))
  .pause(2)

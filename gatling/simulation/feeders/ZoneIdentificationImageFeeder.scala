...
object ZoneIdentificationImageFeeder {

  val zoneIdentificationImageFeed=imageCreator(Test.NUMBER_OF_USERS)
  ...
  def imageCreator(users: Int)={
    ...
    for (id <- 0 until users) {
      ...
      val fileName="ZoneIdentificationImage-"+Random.alphanumeric.take(6).mkString+".jpg"
      ...
      imageFeed(id) = Map(Test.ZONE_IDENTIFICATION_FILENAME -> fileName, Test.ZONE_IDENTIFICATION_FILESIZE -> length.toString)
    }
    imageFeed
  }
}

...
object AssetDetailFeeder {
  ...
  def arrayConstructor(users: Int): Array[Map[String, String]] = {
    ...
    for (id <- 0 until users) {
      val assetPricePerUnit = Random.alphanumeric.filter(_.isDigit).filterNot(x => x == '0').take(3).mkString
      val quantity = Random.alphanumeric.filter(_.isDigit).filterNot(x => x == '0').take(2).mkString

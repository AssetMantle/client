...
case class Split(ownerID: IdentityID, ownableID: OwnableID, value: Long) {

  def send(out: Long): Split = this.copy(value = this.value - out)

  def receive(in: Long): Split = this.copy(value = this.value + in)

  def canSend(out: Long): Boolean = this.value >= out

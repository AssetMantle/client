...
import queries.blockchain.{GetResponse, GetTransaction}
import transactions.Abstract.BaseRequest
import transactions.responses.blockchain.TransactionResponse.{AsyncResponse, BlockResponse, KafkaResponse, SyncResponse}
      ...
      utilitiesOperations.traverse(ticketIDList) { ticketID =>
        ...
        (for {
          ...
          case baseException: BaseException =>
            //TODO Handle the case where tx hash is generated by BC but isn't included in any block.
            if (baseException.failure == constants.Response.TRANSACTION_HASH_NOT_FOUND || baseException.failure == constants.Response.TRANSACTION_NOT_FOUND || baseException.failure.message.matches(awaitingKafkaResponse))

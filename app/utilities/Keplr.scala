...
object Keplr {
  ...
  case class ArbitraryData(account_number: String = "0", chain_id: String = "", fee: Fee = Fee(), memo: String = "", msgs: Seq[Msg], sequence: String = "0") {
    ...
    def getSHA256: Array[Byte] = utilities.Secrets.sha256Hash(Json.toJson(this).toString)

  }

  def newArbitraryData(data: String, signer: String): ArbitraryData = ArbitraryData(msgs = Seq(Msg(value = MsgValue(data = utilities.Secrets.base64URLEncoder(data), signer = signer))))

      ...
      views.companion.master.ChangeBuyerBid.form.bindFromRequest().fold(
        ...
        changeBuyerBidData => {
          try {
            ...
            blockchainTransactionChangeBuyerBids.Service.create(from = username, to = changeBuyerBidData.sellerAddress, bid = changeBuyerBidData.bid, time = changeBuyerBidData.time, pegHash = changeBuyerBidData.pegHash, gas = changeBuyerBidData.gas, null, null, ticketID = ticketID, null)
            if(!kafkaEnabled){
              Future{
                try {
      ...
      changeBuyerBidData => {
        try {
          if (kafkaEnabled) {
            ...
            blockchainTransactionChangeBuyerBids.Service.create(from = changeBuyerBidData.from, to = changeBuyerBidData.to, bid = changeBuyerBidData.bid, time = changeBuyerBidData.time, pegHash = changeBuyerBidData.pegHash, gas = changeBuyerBidData.gas, null, null, ticketID = response.ticketID, null)
            ...
            blockchainTransactionChangeBuyerBids.Service.create(from = changeBuyerBidData.from, to = changeBuyerBidData.to, bid = changeBuyerBidData.bid, time = changeBuyerBidData.time, pegHash = changeBuyerBidData.pegHash, gas = changeBuyerBidData.gas, null, txHash = Option(response.TxHash), ticketID = Random.nextString(32), null)

...
class SendFiatController @Inject()(messagesControllerComponents: MessagesControllerComponents,
    ...
    implicit request =>
      views.companion.master.SendFiat.form.bindFromRequest().fold(
        ...
        sendFiatData => {
          ...
          def createFiatRequest(traderID: String, ticketID: String, negotiationID: String): Future[String] = masterTransactionSendFiatRequests.Service.create(traderID, ticketID, negotiationID, new MicroInt(sendFiatData.sendAmount))
            ...
            if (!loginState.acl.getOrElse(throw new BaseException(constants.Response.UNAUTHORIZED)).sendFiat) throw new BaseException(constants.Response.UNAUTHORIZED)
              ...
              if (validateUsernamePassword) {
                val ticketID = transaction.process[blockchainTransaction.SendFiat, transactionsSendFiat.Request](
                  entity = blockchainTransaction.SendFiat(from = loginState.address, to = sellerAddress, amount = new MicroInt(sendFiatData.sendAmount), pegHash = pegHash, gas = sendFiatData.gas, ticketID = "", mode = transactionMode),
                  blockchainTransactionCreate = blockchainTransactionSendFiats.Service.create,
                  request = transactionsSendFiat.Request(transactionsSendFiat.BaseReq(from = loginState.address, gas = sendFiatData.gas.toString), to = sellerAddress, password = sendFiatData.password, amount = new MicroInt(sendFiatData.sendAmount).value.toString, pegHash = pegHash, mode = transactionMode),
  ...
  def blockchainSendFiat: Action[AnyContent] = withoutLoginActionAsync { implicit request =>
    views.companion.blockchain.SendFiat.form.bindFromRequest().fold(
      ...
      sendFiatData => {
        val postRequest = transactionsSendFiat.Service.post(transactionsSendFiat.Request(transactionsSendFiat.BaseReq(from = sendFiatData.from, gas = sendFiatData.gas.toString), to = sendFiatData.to, password = sendFiatData.password, amount = new MicroInt(sendFiatData.sendAmount).value.toString, pegHash = sendFiatData.pegHash, mode = sendFiatData.mode))
        (for {
          _ <- postRequest

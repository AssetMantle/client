        ...
        try {
          ...
          userType match {
            ...
            case constants.User.TRADER =>
              ...
              withUsernameToken.Ok(views.html.component.master.traderHome(username = loginData.username, userType = constants.User.TRADER, address = address, coins = blockchainAccounts.Service.getCoins(address), assetPegWallet = blockchainAssets.Service.getAssetPegWallet(address), fiatPegWallet = blockchainFiats.Service.getFiatPegWallet(owners.map(_.pegHash).seq), totalFiat = owners.map(_.amount.toInt).sum, zone = masterZones.Service.getZone(aclAccount.zoneID), organization = masterOrganizations.Service.getOrganization(aclAccount.organizationID), aclHash = blockchainAclHashes.Service.getACLHash(aclAccount.aclHash)), loginData.username)

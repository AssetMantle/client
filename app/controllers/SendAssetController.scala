        ...
        sendAssetData => {
          try {
            if (kafkaEnabled) {
              ...
              sendAssets.Service.addSendAssetKafka(from = username, to = sendAssetData.to, pegHash = sendAssetData.pegHash, gas = sendAssetData.gas, null, null, ticketID = response.ticketID, null)
              ...
              sendAssets.Service.addSendAsset(from = username, to = sendAssetData.to, pegHash = sendAssetData.pegHash, gas = sendAssetData.gas, null, txHash = Option(response.TxHash), ticketID = Random.nextString(32), null)

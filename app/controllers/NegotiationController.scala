...
class NegotiationController @Inject()(
  ...
  def updateAssetTermsForm(id: String): Action[AnyContent] = withTraderLoginAction.authenticated { implicit loginState =>
    implicit request =>
      ...
      def getResult(traderID: String, negotiation: Negotiation): Future[Result] = if (traderID == negotiation.sellerTraderID) {
        withUsernameToken.Ok(views.html.component.master.updateNegotiationAssetTerms(views.companion.master.UpdateNegotiationAssetTerms.form.fill(views.companion.master.UpdateNegotiationAssetTerms.Data(id = negotiation.id, description = negotiation.assetDescription, pricePerUnit = new MicroLong(roundAtTwoDecimal(negotiation.price.realDouble / negotiation.quantity.realDouble)), quantity = negotiation.quantity, quantityUnit = negotiation.quantityUnit, gas = constants.FormField.GAS.maximumValue, password = ""))))
      } else {
        throw new BaseException(constants.Response.UNAUTHORIZED)
      }
          ...
          def updateAssetTermsAndGetResult(validateUsernamePassword: Boolean, traderID: String, buyerAccountID: String, negotiation: Negotiation): Future[Result] = {
            if (traderID == negotiation.sellerTraderID) {
              if (validateUsernamePassword) {
                ...
                val updatePrice = if (new MicroLong(roundAtTwoDecimal(updateAssetTermsData.pricePerUnit.realDouble * updateAssetTermsData.quantity.realDouble)) != negotiation.price) {
                  for {
                    pegHash <- getPegHash(negotiation.assetID)
                    buyerAddress <- getAddress(buyerAccountID)
                    ticketID <- sendTransaction(buyerAddress = buyerAddress, pegHash = pegHash, price = new MicroLong(roundAtTwoDecimal(updateAssetTermsData.pricePerUnit.realDouble * updateAssetTermsData.quantity.realDouble)))
                  } yield 0
                } else Future(0)
              ...
              } else Future(BadRequest(views.html.component.master.updateNegotiationAssetTerms(views.companion.master.UpdateNegotiationAssetTerms.form.fill(views.companion.master.UpdateNegotiationAssetTerms.Data(id = negotiation.id, description = negotiation.assetDescription, pricePerUnit = new MicroLong(roundAtTwoDecimal(negotiation.price.realDouble / negotiation.quantity.realDouble)), quantity = negotiation.quantity, quantityUnit = negotiation.quantityUnit, gas = constants.FormField.GAS.maximumValue, password = "")).withGlobalError(constants.Response.INCORRECT_PASSWORD.message))))
            } else throw new BaseException(constants.Response.UNAUTHORIZED)
          }

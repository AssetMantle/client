...
class NegotiationController @Inject()(
  ...
  def reviewRequestForm(id: String): Action[AnyContent] = withTraderLoginAction.authenticated { implicit loginState =>
    implicit request =>
      ...
      (for {
        ...
        counterPartyTrader <- getTrader(negotiation.buyerTraderID)
      } yield Ok(views.html.component.master.reviewNegotiationRequest(asset = asset, negotiation = negotiation, counterPartyTrader = counterPartyTrader))
        ).recover {
        case baseException: BaseException => InternalServerError(views.html.trades(failures = Seq(baseException.failure)))
      }
  }
    ...
    implicit request =>
      ...
      (for {
        ...
        if (traderID == negotiation.buyerTraderID) {
          Ok(views.html.component.master.acceptNegotiationRequest(negotiation = negotiation, sellerAccountID = sellerAccountID))
        } else {
          throw new BaseException(constants.Response.UNAUTHORIZED)
        }
      }
          ...
          (loginState.acl match {
            case Some(acl) =>
              if (acl.changeBuyerBid) {
                ...
                def acceptNegotiationAndGetResult(validateUsernamePassword: Boolean, negotiation: Negotiation, sellerAccountID: String): Future[Result] =
                  if (validateUsernamePassword) {
                    ...
                    Future(BadRequest(views.html.component.master.acceptNegotiationRequest(views.companion.master.AcceptNegotiationRequest.form.fill(acceptRequestData).withGlobalError(constants.Response.INCORRECT_PASSWORD.message), negotiation = negotiation, sellerAccountID = sellerAccountID)))
                  }
                ...
                for {
                  ...
                  negotiation <- negotiation
                  sellerAccountID <- getTraderAccountID(negotiation.sellerTraderID)
                  result <- acceptNegotiationAndGetResult(validateUsernamePassword = validateUsernamePassword, negotiation = negotiation, sellerAccountID = sellerAccountID)
                } yield result
              } else throw new BaseException(constants.Response.UNAUTHORIZED)
  ...
  def rejectRequestForm(id: String): Action[AnyContent] = withTraderLoginAction.authenticated { implicit loginState =>
    implicit request =>
      ...
      (for {
        ...
        if (traderID == negotiation.buyerTraderID) {
          Ok(views.html.component.master.rejectNegotiationRequest(negotiation = negotiation, sellerAccountID = sellerAccountID))
        } else {
          throw new BaseException(constants.Response.UNAUTHORIZED)
        }
      }
  ...
  def acceptOrRejectNegotiationTermsForm(id: String, termType: String): Action[AnyContent] = withTraderLoginAction.authenticated { implicit loginState =>
    implicit request =>
      ...
      (for {
        ...
        if (traderID == negotiation.buyerTraderID) {
          Ok(views.html.component.master.acceptOrRejectNegotiationTerms(negotiationID = negotiation.id, termType = termType, status = false))
        } else {
          throw new BaseException(constants.Response.UNAUTHORIZED)
        }
      }
    ...
    implicit request =>
      ...
      (for {
        ...
        documentContent match {
          ...
          case None => Ok(views.html.component.master.addInvoice(negotiationID = negotiationID, negotiation = negotiation, traderList = traderList, organizationList = organizationList))
        }
      }
        ).recover {
      ...
      (for {
        ...
        documentContent match {
          case Some(content) => {
            ...
            Ok(views.html.component.master.addContract(views.companion.master.AddContract.form.fill(views.companion.master.AddContract.Data(negotiationID = negotiationID, contractNumber = contract.contractNumber)), negotiationID = negotiationID))
          }
          case None => Ok(views.html.component.master.addContract(negotiationID = negotiationID))
        }
      }
        ).recover {
        case baseException: BaseException => InternalServerError(views.html.tradeRoom(negotiationID = negotiationID, failures = Seq(baseException.failure)))
        ...
        buyerConfirmData => {
          ...
          def getResult(negotiation: Negotiation, negotiationDocumentList: Seq[NegotiationFile], assetDocumentList: Seq[AssetFile]): Future[Result] = {
            if (negotiation.documentList.negotiationDocuments.diff(negotiationDocumentList.filterNot(_.documentType == constants.File.Negotiation.CONTRACT).map(_.documentType)).isEmpty && negotiation.documentList.assetDocuments.diff(assetDocumentList.map(_.documentType)).isEmpty) {
              if (assetDocumentList.find(_.documentType == constants.File.Asset.BILL_OF_LADING).map(billOfLading => billOfLading.status == Option(true) && (if (billOfLading.documentContent.isDefined) true else throw new BaseException(constants.Response.BILL_OF_LADING_CONTENT_NOT_FOUND))).getOrElse(throw new BaseException(constants.Response.BILL_OF_LADING_NOT_FOUND))) {
                ...
                def sendTransactionAndGetResult(validateUsernamePassword: Boolean, sellerAccountID: String, buyerTraderID: String, sellerAddress: String, pegHash: String, negotiation: Negotiation, contract: NegotiationFile): Future[Result] = {
                  if (validateUsernamePassword) {
          ...
          def getResult(negotiation: Negotiation, negotiationDocumentList: Seq[NegotiationFile], assetDocumentList: Seq[AssetFile]): Future[Result] = {
            if (negotiation.documentList.negotiationDocuments.diff(negotiationDocumentList.filterNot(_.documentType == constants.File.Negotiation.CONTRACT).map(_.documentType)).isEmpty && negotiation.documentList.assetDocuments.diff(assetDocumentList.map(_.documentType)).isEmpty) {
              if (assetDocumentList.find(_.documentType == constants.File.Asset.BILL_OF_LADING).map(billOfLading => billOfLading.status == Option(true) && (if (billOfLading.documentContent.isDefined) true else throw new BaseException(constants.Response.BILL_OF_LADING_CONTENT_NOT_FOUND))).getOrElse(throw new BaseException(constants.Response.BILL_OF_LADING_NOT_FOUND))) {
                ...
                def sendTransactionAndGetResult(validateUsernamePassword: Boolean, buyerAccountID: String, sellerTraderID: String, buyerAddress: String, pegHash: String, negotiation: Negotiation, contract: NegotiationFile): Future[Result] = {
                  if (validateUsernamePassword) {
  ...
  def confirmAllNegotiationTermsForm(negotiationID: String): Action[AnyContent] = withTraderLoginAction.authenticated { implicit loginState =>
    implicit request =>
      Future(Ok(views.html.component.master.confirmAllNegotiationTerms(negotiationID = negotiationID)))
  }

...
@Singleton
class WithXmlParseActionLoggingFilter @Inject()(messagesControllerComponents: MessagesControllerComponents, messagesApi: MessagesApi, utilitiesLog: utilities.Log)(implicit executionContext: ExecutionContext, configuration: Configuration) extends AbstractController(messagesControllerComponents) with I18nSupport {

  private implicit val lang = Lang(configuration.get[String]("play.log.lang"))

  def next(f: => Request[NodeSeq] => Future[Result])(implicit logger: Logger): Action[NodeSeq] = Action.async(parse.xml) { implicit request ⇒

        ...
        setACLData => {
          try {
            if (masterOrganizations.Service.getStatus(setACLData.organizationID) == Option(true)) {
              ...
              blockchainTransactionSetACLs.Service.addSetACL(from = username, aclAddress = setACLData.aclAddress, organizationID = setACLData.organizationID, zoneID = zoneID, acl = acl, null, txHash = null, ticketID = ticketID, null)
              if (!kafkaEnabled) {
                Future {
                  try {

    ...
    try {
      ...
      account.userType match {
        case constants.User.UNKNOWN =>
          Ok(views.html.component.master.commonHome(username, account.userType, account.accountAddress))
        case _ =>
          Ok(views.html.component.master.commonHome(username, account.userType, account.accountAddress, blockchainAccounts.Service.getCoins(account.accountAddress)))
      }
    } catch {

        ...
        verifyZoneData => {
          try {
            if (kafkaEnabled) {
              ...
              blockchainZones.Service.addZone(verifyZoneData.zoneID, zoneAccountAddress, dirtyBit = false)
              masterZones.Service.updateStatus(verifyZoneData.zoneID, status = true)

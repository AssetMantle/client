...
class ComponentViewController @Inject()(
  ...
  def traderViewAcceptedNegotiationTerms(negotiationID: String): Action[AnyContent] = withTraderLoginAction.authenticated { implicit loginState =>
    implicit request =>
      ...
      def getZoneAccountID(zoneID:String) =  masterZones.Service.tryGetAccountID(zoneID)

      def getZoneAddress(zoneAccountID:String) = blockchainAccounts.Service.tryGetAddress(zoneAccountID)

      def getSuccessiveTransactionStatus(traderID: String, negotiation: Negotiation, order: Option[Order], buyerAddress: String, sellerAddress: String, zoneAddress:String, asset: Asset) = {
        ...
        if (traderID == negotiation.buyerTraderID) {
          negotiation.status match {
            ...
            case constants.Status.Negotiation.BOTH_PARTIES_CONFIRMED | constants.Status.Negotiation.COMPLETED => order match {
              case Some(order) => {
                order.status match {
                  ...
                  case constants.Status.Order.COMPLETED => {
                    asset.status match {
                      case constants.Status.Asset.TRADED => blockchainTransactionRedeemAssets.Service.getTransactionStatus(buyerAddress, zoneAddress, pegHash)
      ...
      (for {
        ...
        counterPartyTrader <- getCounterPartyTrader(trader.id, negotiation)
        buyerAddress <- getBuyerAddress(trader.id, negotiation, counterPartyTrader.accountID)
        sellerAddress <- getSellerAddress(trader.id, negotiation, counterPartyTrader.accountID)
        zoneAccountID <- getZoneAccountID(if(trader.id == negotiation.buyerTraderID) trader.zoneID else counterPartyTrader.zoneID)
        zoneAddress<- getZoneAddress(zoneAccountID)
        successiveTransactionStatus <- getSuccessiveTransactionStatus(trader.id, negotiation, order, buyerAddress, sellerAddress, zoneAddress, asset)
        result <- getResult(trader.id, negotiation, order, asset, counterPartyTrader, successiveTransactionStatus)
      } yield result
        ).recover {
        case baseException: BaseException => InternalServerError(baseException.failure.message)
      }
  ...
  def zoneViewTradeRoomFinancial(negotiationID: String): Action[AnyContent] = withZoneLoginAction.authenticated { implicit loginState =>
    implicit request =>
      ...
      (for {
        ...
        asset <- getAsset(negotiation.assetID)
      } yield Ok(views.html.component.master.zoneViewTradeRoomFinancial(fiatsInOrder, 0, negotiation.price - fiatsInOrder, asset.moderated))
        ).recover {
        case baseException: BaseException => InternalServerError(baseException.failure.message)
      }
  }
  ...
  def zoneViewCompletedTradeRoomFinancial(negotiationID: String): Action[AnyContent] = withZoneLoginAction.authenticated { implicit loginState =>
    implicit request =>
      ...
      def getAssetHistory(assetID: String) = masterAssetHistories.Service.tryGet(assetID)
      ...
      (for {
        ...
        traderIDList <- getTraderIDList(zoneID)
        fiatPegWallet <- getFiatPegWallet(traderIDList)
        assetHistory <- getAssetHistory(negotiationHistory.assetID)
      } yield Ok(views.html.component.master.zoneViewCompletedTradeRoomFinancial(fiatPegWallet.map(_.transactionAmount).sum, 0, negotiationHistory.price - fiatsInOrderHistory, assetHistory.moderated))
        ).recover {
        case baseException: BaseException => InternalServerError(baseException.failure.message)
      }
  }
  ...
  def organizationViewTradeRoomFinancial(negotiationID: String): Action[AnyContent] = withOrganizationLoginAction.authenticated { implicit loginState =>
    implicit request =>
      ...
      (for {
        ...
        fiatPegWallet <- getFiatPegWallet(traderIDList)
        asset <- getAsset(negotiation.assetID)
      } yield Ok(views.html.component.master.organizationViewTradeRoomFinancial(fiatPegWallet.map(_.transactionAmount).sum, 0, negotiation.price - fiatsInOrder, asset.moderated))
        ).recover {
        case baseException: BaseException => InternalServerError(baseException.failure.message)
      }
  }
  ...
  def organizationViewCompletedTradeRoomFinancial(negotiationID: String): Action[AnyContent] = withOrganizationLoginAction.authenticated { implicit loginState =>
    implicit request =>
      ...
      (for {
        ...
        fiatPegWallet <- getFiatPegWallet(traderIDList)
        assetHistory <- getAssetHistory(negotiationHistory.assetID)
      } yield Ok(views.html.component.master.organizationViewTradeRoomFinancial(fiatPegWallet.map(_.transactionAmount).sum, 0, negotiationHistory.price - fiatsInOrderHistory, assetHistory.moderated))
        ).recover {
        case baseException: BaseException => InternalServerError(baseException.failure.message)
      }
  }
  ...
  def traderViewTradeRoomFinancial(negotiationID: String): Action[AnyContent] = withTraderLoginAction.authenticated { implicit loginState =>
    implicit request =>
      ...
      (for {
        ...
        fiatPegWallet <- getFiatPegWallet(traderID)
        asset <- getAsset(negotiation.assetID)
      } yield Ok(views.html.component.master.traderViewTradeRoomFinancial(walletBalance = fiatPegWallet.map(_.transactionAmount).sum, amountPaid = 0, amountPending = (negotiation.price - fiatsInOrder), moderated = asset.moderated,negotiationID = negotiationID))
  ...
  def traderViewCompletedTradeRoomFinancial(negotiationID: String): Action[AnyContent] = withTraderLoginAction.authenticated { implicit loginState =>
    implicit request =>
      ...
      (for {
        ...
        fiatPegWallet <- getFiatPegWallet(traderID)
        asset <- getAsset(negotiation.assetID)
      } yield Ok(views.html.component.master.traderViewCompletedTradeRoomFinancial(fiatPegWallet.map(_.transactionAmount).sum, 0, negotiation.price - fiatsInOrderHistory, asset.moderated))
        ).recover {
        case baseException: BaseException => InternalServerError(baseException.failure.message)
      }
  }
  ...
  def traderViewCompletedNegotiationDocumentContent(negotiationID: String, documentType: String): Action[AnyContent] = withTraderLoginAction.authenticated { implicit loginState =>
    implicit request =>
  ...
  def traderViewIssueFiatRequestList: Action[AnyContent] = withTraderLoginAction.authenticated {
    implicit loginState =>
      implicit request =>

      val traderID = masterTraders.Service.tryGetID(loginState.username)

      def getIssueFiatRequestList(traderID: String) =
          wallexWalletTransferRequests.Service.getAllByTraderID(traderID)
  ...
  def organizationViewIssueFiatRequestList: Action[AnyContent] = withOrganizationLoginAction.authenticated {
    implicit loginState =>
      implicit request =>

      val organizationID = masterOrganizations.Service.tryGetID(loginState.username)

      def getIssueFiatRequestList(organizationID: String) =
          wallexWalletTransferRequests.Service.getAllByOrganizationID(organizationID)
  ...
  def zoneViewIssueFiatRequestList: Action[AnyContent] = withZoneLoginAction.authenticated {
    implicit loginState =>
      implicit request =>
      ...
      def getIssueFiatRequestList(organizationIDs: Seq[String]) =
          wallexWalletTransferRequests.Service.getAllByOrganizationIDs(organizationIDs)
  ...
  def traderViewOrganizationWallexAccount : Action[AnyContent] = withTraderLoginAction.authenticated { implicit loginState =>
    implicit request =>
      ...
      def getOrganizationWallexAccount(organizationID: String): Future[Option[OrganizationAccount]] =
        wallexOrganizationAccounts.Service.getByOrganizationID(organizationID)
  ...
  def traderWallexPaymentTransfer(negotiationID: String) : Action[AnyContent] = withTraderLoginAction.authenticated { implicit loginState =>
    implicit request =>
      ...
      (for {
        organizationID <- organizationID
        organizationWallexAccount <- getOrganizationWallexAccount(organizationID)
      } yield Ok(views.html.component.wallex.traderTransferWalletPayment(organizationWallexAccount,negotiationID))
        ).recover {
        case baseException: BaseException => InternalServerError(baseException.failure.message)
      }
  }
  ...
  def traderWallexTransferToBeneficiary(negotiationID: String): Action[AnyContent] = withTraderLoginAction.authenticated { implicit loginState =>
    implicit request =>
      ...
      def getOrganizationWallexAccount(organizationID: String): Future[OrganizationAccount] =
        wallexOrganizationAccounts.Service.tryGetByOrganizationID(organizationID)
  ...
  def traderViewBeneficiary : Action[AnyContent] = withTraderLoginAction.authenticated { implicit loginState =>
    implicit request =>
      val organizationID = masterTraders.Service.getOrganizationIDByAccountID(loginState.username)
    ...
    withZoneLoginAction.authenticated {
      implicit loginState =>
        implicit request =>
          ...
          def pendingWalletTransferRequests(organizationIDs: Seq[String]): Future[Seq[WalletTransferRequest]] =
            wallexWalletTransferRequests.Service.tryGetPendingRequests(organizationIDs)
          ...
          (for {
            zoneID <- zoneID
            organizations <- getOrganizations(zoneID)
            pendingWalletRequests <- pendingWalletTransferRequests(getOrganizationIDs(organizations))
          } yield Ok(

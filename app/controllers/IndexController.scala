    ...
    try {
      ...
      userType match {
        case constants.User.GENESIS =>
          withUsernameToken.Ok(views.html.genesisHome(username = username, address = address, coins = blockchainAccounts.Service.getCoins(address),profilePicture = masterAccountFiles.Service.getProfilePicture(username)), username)
        case constants.User.ZONE =>
          withUsernameToken.Ok(views.html.zoneHome(username = username, address = address, coins = blockchainAccounts.Service.getCoins(address), zone = masterZones.Service.get(blockchainZones.Service.getID(address)),profilePicture = masterAccountFiles.Service.getProfilePicture(username)), username)
        case constants.User.ORGANIZATION =>
          withUsernameToken.Ok(views.html.organizationHome(username = username, address = address, coins = blockchainAccounts.Service.getCoins(address), organization = masterOrganizations.Service.get(blockchainOrganizations.Service.getID(address)),profilePicture = masterAccountFiles.Service.getProfilePicture(username)), username)

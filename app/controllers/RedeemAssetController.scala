        ...
        redeemAssetData => {
          try {
            if (kafkaEnabled) {
              ...
              redeemAssets.Service.addRedeemAssetKafka(from = username, to = redeemAssetData.to, pegHash = redeemAssetData.pegHash, gas = redeemAssetData.gas, null, null, ticketID = response.ticketID, null)
              ...
              redeemAssets.Service.addRedeemAsset(from = username, to = redeemAssetData.to, pegHash = redeemAssetData.pegHash, gas = redeemAssetData.gas, null, txHash = Option(response.TxHash), ticketID = Random.nextString(32), null)

      ...
      redeemAssetData => {
        try {
          if (kafkaEnabled) {
            ...
            blockchainTransactionRedeemAssets.Service.create(from = redeemAssetData.from, to = redeemAssetData.to, pegHash = redeemAssetData.pegHash, gas = redeemAssetData.gas, null, null, ticketID = response.ticketID, null)
            ...
            blockchainTransactionRedeemAssets.Service.create(from = redeemAssetData.from, to = redeemAssetData.to, pegHash = redeemAssetData.pegHash, gas = redeemAssetData.gas, null, txHash = Option(response.TxHash), ticketID = Random.nextString(32), null)

      ...
      releaseAssetData => {
        try {
          if (kafkaEnabled) {
            ...
            blockchainTransactionReleaseAssets.Service.create(from = releaseAssetData.from, to = releaseAssetData.to, pegHash = releaseAssetData.pegHash, gas = releaseAssetData.gas, null, null, ticketID = response.ticketID, null)
            ...
            blockchainTransactionReleaseAssets.Service.create(from = releaseAssetData.from, to = releaseAssetData.to, pegHash = releaseAssetData.pegHash, gas = releaseAssetData.gas, null, txHash = Option(response.TxHash), ticketID = Random.nextString(32), null)

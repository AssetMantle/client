...
class Fiats @Inject()(
  ...
  private def updateDirtyBitByAddress(address: String, dirtyBit: Boolean): Future[Int] = db.run(fiatTable.filter(_.ownerAddress === address).map(_.dirtyBit).update(dirtyBit).asTry).map {
    case Success(result) => if (result > 0) result else {
      val create = add(serialize(Fiat(pegHash = utilities.IDGenerator.hexadecimal, address, "", new MicroLong(0), new MicroLong(0), true)))
      for {
        _ <- create
      } yield 1
      1
  ...
  object Service {
    ...
    def create(pegHash: String, ownerAddress: String, transactionID: String, transactionAmount: MicroLong, redeemedAmount: MicroLong, dirtyBit: Boolean): Future[String] = add(serialize(Fiat(pegHash, ownerAddress, transactionID, transactionAmount, redeemedAmount, dirtyBit)))
            ...
            accountResponse.value.fiat_peg_wallet match {
              case Some(updatedFiatPegWallet) =>
                ...
                val updateFiats = Future.traverse(oldFiatPegWallet.map(_.pegHash).intersect(updatedFiatPegWallet.map(_.pegHash)).flatMap(pegHash => updatedFiatPegWallet.find(_.pegHash == pegHash)))(fiatPeg => {
                  Service.update(Fiat(pegHash = fiatPeg.pegHash, ownerAddress = dirtyFiat.ownerAddress, transactionID = fiatPeg.transactionID, transactionAmount = new MicroLong(fiatPeg.transactionAmount.toLong), redeemedAmount = new MicroLong(fiatPeg.redeemedAmount.toLong), dirtyBit = false))
                })
                val insertFiats = Future.traverse(updatedFiatPegWallet.map(_.pegHash).diff(oldFiatPegWallet.map(_.pegHash)).flatMap(pegHash => updatedFiatPegWallet.find(_.pegHash == pegHash)))(fiatPeg => {
                  Service.create(pegHash = fiatPeg.pegHash, ownerAddress = dirtyFiat.ownerAddress, transactionID = fiatPeg.transactionID, transactionAmount =new MicroLong(fiatPeg.transactionAmount.toLong), redeemedAmount =new MicroLong(fiatPeg.redeemedAmount.toLong), dirtyBit = false)
                })
                for {
                  _ <- deleteFiats
                  _ <- updateFiats
                  ...
                  def upsert(traderID: String) =
                    accountOwnerAddress.value.fiat_peg_wallet match {
                      case Some(updatedFiatPegWallet) => {
                        ...
                        val updateFiats = Future.traverse(oldFiatPegWallet.map(_.pegHash).intersect(updatedFiatPegWallet.map(_.pegHash)).flatMap(pegHash => updatedFiatPegWallet.find(_.pegHash == pegHash)))(fiatPeg => {
                          masterFiats.Service.updateFiat(traderID, fiatPeg.transactionID, new MicroLong(fiatPeg.transactionAmount.toLong), new MicroLong(fiatPeg.redeemedAmount.toLong))
                        })
                        val insertFiats = Future.traverse(updatedFiatPegWallet.map(_.pegHash).diff(oldFiatPegWallet.map(_.pegHash)).flatMap(pegHash => updatedFiatPegWallet.find(_.pegHash == pegHash)))(fiatPeg => {
                          masterFiats.Service.insertOrUpdate(traderID, fiatPeg.transactionID, new MicroLong(fiatPeg.transactionAmount.toLong), new MicroLong(fiatPeg.redeemedAmount.toLong))
                        })
                        for {

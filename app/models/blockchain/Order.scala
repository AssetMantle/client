        ...
        Future.sequence {
          dirtyOrders.map { dirtyOrder =>
            ...
            def completeOrReverseOrder(masterOrder: masterOrder, masterNegotiation: masterNegotiation, negotiation: Negotiation, assetPegWallet: Seq[Asset], fiatPegWallet: Seq[Fiat], orderResponse: OrderResponse.Response): Future[Unit] = {
              val fiatsInOrder = if (fiatPegWallet.nonEmpty) fiatPegWallet.map(_.transactionAmount).map(_.value).sum else BigInt(0)
              if (orderResponse.value.awb_proof_hash != "" && orderResponse.value.fiat_proof_hash != "") {
                ...
                val createReceiveFiat = masterTransactionReceiveFiats.Service.create(masterNegotiation.buyerTraderID, masterOrder.id, new MicroNumber(fiatsInOrder), constants.Status.ReceiveFiat.ORDER_REVERSED_FIAT)
                for {
                  _ <- updateAsset
                  _ <- buyerMarkDirty
                  _ <- deleteOrderFiats

...
case class OrderNew(id: String, immutables: Immutables, mutables: Mutables, createdBy: Option[String] = None, createdOn: Option[Timestamp] = None, createdOnTimeZone: Option[String] = None, updatedBy: Option[String] = None, updatedOn: Option[Timestamp] = None, updatedOnTimeZone: Option[String] = None) extends Logged {
...
class OrdersNew @Inject()(
  ...
  private def addMultiple(orders: Seq[OrderNew]): Future[Seq[String]] = db.run((orderTable returning orderTable.map(_.id) ++= orders.map(x => serialize(x))).asTry).map {
    case Success(result) => result
    case Failure(exception) => exception match {
      case psqlException: PSQLException => throw new BaseException(constants.Response.ORDER_INSERT_FAILED, psqlException)
    }
  ...
  private def upsert(order: OrderNew): Future[Int] = db.run(orderTable.insertOrUpdate(serialize(order)).asTry).map {
    case Success(result) => result
    case Failure(exception) => exception match {
  ...
  object Service {
    ...
    def create(order: OrderNew): Future[String] = add(order)

    def tryGet(id: String): Future[OrderNew] = tryGetByID(id).map(_.deserialize)

    def get(id: String): Future[Option[OrderNew]] = getByID(id).map(_.map(_.deserialize))

    def getAll: Future[Seq[OrderNew]] = getAllOrders.map(_.map(_.deserialize))

    def insertMultiple(orders: Seq[OrderNew]): Future[Seq[String]] = addMultiple(orders)

    def insertOrUpdate(order: OrderNew): Future[Int] = upsert(order)
  ...
  object Utility {
    ...
    def onMake(orderMake: OrderMake, blockHeight: Int): Future[Unit] = {
      ...
      def getNewMakerOwnableSplit(oldOrder: Option[OrderNew]): Future[BigDecimal] = if (oldOrder.isDefined) {
      ...
      def upsertOrder(oldOrder: Option[OrderNew], scrubbedMutables: Seq[Property], orderID: String, immutables: Immutables) = {
        val mutables = Mutables(Properties(scrubbedMutables ++ orderMake.mutableProperties.propertyList))
        oldOrder.fold(Service.insertOrUpdate(OrderNew(id = orderID, mutables = mutables, immutables = immutables)))(x => Service.insertOrUpdate(x.copy(mutables = x.mutables.mutate(mutables.properties.propertyList))))
      }

      def masterOperations(orderID: String) = {
        val insert = masterOrdersNew.Service.insertOrUpdate(master.OrderNew(id = orderID, makerOwnableID = orderMake.makerOwnableID, takerOwnableID = orderMake.takerOwnableID, makerID = orderMake.fromID, status = Option(true)))
    ...
    def onTake(orderTake: OrderTake): Future[Unit] = {
      ...
      def updateOrRemoveOrder(oldOrder: OrderNew, makerOwnableSplit: BigDecimal, exchangeRate: BigDecimal) = {
      ...
      def transferSplits(oldOrder: OrderNew, sendTakerOwnableSplit: BigDecimal, sendMakerOwnableSplit: BigDecimal) = {
      ...
      def masterOperations(orderID: String, orderDeleted: Boolean, metaMutables: Seq[MetaProperty]): Future[Unit] = {
        if (orderDeleted) {
          val deleteOrder = masterOrdersNew.Service.delete(orderID)
    ...
    def onCancel(orderCancel: OrderCancel): Future[Unit] = {
      ...
      def getMakerOwnableSplitData(oldOrder: OrderNew) = blockchainMetas.Service.tryGetData(id = oldOrder.getMakerOwnableSplit.fact.hash, dataType = constants.Blockchain.DataType.DEC_DATA)

      def auxiliaryTransfer(oldOrder: OrderNew, makerOwnableSplit: BigDecimal) = blockchainSplits.Utility.auxiliaryTransfer(fromID = constants.Blockchain.Modules.Orders, toID = orderCancel.fromID, ownableID = oldOrder.getMakerOwnableID, splitValue = makerOwnableSplit)

        ...
        Future.sequence {
          dirtyNegotiations.map { dirtyNegotiation =>
            ...
            def updateMasterNegotiationStatus(negotiation: masterNegotiation, negotiationResponse: Response): Future[Int] = {
              if (negotiationResponse.value.buyerSignature.isEmpty && negotiationResponse.value.sellerSignature.isEmpty) masterNegotiations.Service.update(negotiation.copy(price = new MicroInt(negotiationResponse.value.bid.toLong), buyerAcceptedPrice = if (negotiation.status == constants.Status.Negotiation.STARTED) false else negotiation.buyerAcceptedPrice, time = Option(negotiationResponse.value.time.toInt)))
              else if (negotiationResponse.value.buyerSignature.isDefined && negotiationResponse.value.sellerSignature.isEmpty) masterNegotiations.Service.update(negotiation.copy(price = new MicroInt(negotiationResponse.value.bid.toLong), time = Option(negotiationResponse.value.time.toInt), status = constants.Status.Negotiation.BUYER_CONFIRMED_SELLER_PENDING))
              else if (negotiationResponse.value.buyerSignature.isEmpty && negotiationResponse.value.sellerSignature.isDefined) masterNegotiations.Service.update(negotiation.copy(price = new MicroInt(negotiationResponse.value.bid.toLong), time = Option(negotiationResponse.value.time.toInt), status = constants.Status.Negotiation.SELLER_CONFIRMED_BUYER_PENDING))
              else if (negotiationResponse.value.buyerSignature.isDefined && negotiationResponse.value.sellerSignature.isDefined) masterNegotiations.Service.update(negotiation.copy(price = new MicroInt(negotiationResponse.value.bid.toLong), time = Option(negotiationResponse.value.time.toInt), status = constants.Status.Negotiation.BOTH_PARTIES_CONFIRMED))
              else Future(0)
            }

...
class Validators @Inject()(
  ...
  object Service {
    ...
    def insertMultipleValidatorsWithActor(validators: Seq[Validator]): Future[Seq[String]] = (validatorActorRegion ? InsertMultipleValidators(uniqueId, validators)).mapTo[Seq[String]]

    def insertMultiple(validators: Seq[Validator]): Future[Seq[String]] = addMultiple(validators)

    def insertOrUpdateValidatorWithActor(validator: Validator): Future[Int] = (validatorActorRegion ? InsertOrUpdateValidator(uniqueId, validator)).mapTo[Int]

    def insertOrUpdate(validator: Validator): Future[Int] = upsert(validator)

    def tryGetValidatorWithActor(address: String): Future[Validator] = (validatorActorRegion ? TryGetValidator(uniqueId, address)).mapTo[Validator]

    def tryGet(address: String): Future[Validator] = tryGetValidatorByOperatorOrHexAddress(address).map(_.deserialize)

    def getAllValidatorByHexAddressesWithActor(hexAddresses: Seq[String]): Future[Seq[Validator]] = (validatorActorRegion ? GetAllValidatorByHexAddresses(uniqueId, hexAddresses)).mapTo[Seq[Validator]]
    ...
    def tryGetValidatorByOperatorAddressWithActor(operatorAddress: String): Future[Seq[Validator]] = (validatorActorRegion ? TryGetValidatorByOperatorAddress(uniqueId, operatorAddress)).mapTo[Seq[Validator]]
    ...
    def tryGetValidatorByHexAddressWithActor(hexAddress: String): Future[Validator] = (validatorActorRegion ? TryGetValidatorByHexAddress(uniqueId, hexAddress)).mapTo[Validator]

    def tryGetByHexAddress(hexAddress: String): Future[Validator] = tryGetValidatorByHexAddress(hexAddress).map(_.deserialize)

    def tryGetValidatorOperatorAddressWithActor(hexAddress: String): Future[String] = (validatorActorRegion ? TryGetValidatorOperatorAddress(uniqueId, hexAddress)).mapTo[String]

    def tryGetOperatorAddress(hexAddress: String): Future[String] = tryGetOperatorAddressByHexAddress(hexAddress)

    def tryGetValidatorHexAddressWithActor(operatorAddress: String): Future[String] = (validatorActorRegion ? TryGetValidatorHexAddress(uniqueId, operatorAddress)).mapTo[String]

    def tryGetHexAddress(operatorAddress: String): Future[String] = tryGethexAddressByOperatorAddress(operatorAddress)

    def tryGetProposerNameWithActor(hexAddress: String): Future[String] = (validatorActorRegion ? TryGetValidatorProposerName(uniqueId, hexAddress)).mapTo[String]
    ...
    def getAllValidatorsWithActor: Future[Seq[Validator]] = (validatorActorRegion ? GetAllValidators(uniqueId)).mapTo[Seq[Validator]]

    def getAll: Future[Seq[Validator]] = getAllValidators.map(_.map(_.deserialize))

    def getAllActiveValidatorListWithActor: Future[Seq[Validator]] = (validatorActorRegion ? GetAllActiveValidatorList(uniqueId)).mapTo[Seq[Validator]]
    ...
    def getAllInActiveValidatorListWithActor: Future[Seq[Validator]] = (validatorActorRegion ? GetAllInactiveValidatorList(uniqueId)).mapTo[Seq[Validator]]

    def getAllInactiveValidatorList: Future[Seq[Validator]] = getAllInactiveValidators.map(_.map(_.deserialize))

    def getAllUnbondingValidatorListWithActor: Future[Seq[Validator]] = (validatorActorRegion ? GetAllUnbondingValidatorList(uniqueId)).mapTo[Seq[Validator]]
    ...
    def getAllUnbondedValidatorListWithActor: Future[Seq[Validator]] = (validatorActorRegion ? GetAllUnbondedValidatorList(uniqueId)).mapTo[Seq[Validator]]
    ...
    def getByOperatorAddressesWithActor(operatorAddresses: Seq[String]): Future[Seq[Validator]] = (validatorActorRegion ? GetValidatorByOperatorAddresses(uniqueId, operatorAddresses)).mapTo[Seq[Validator]]
    ...
    def existsValidatorWithActor(operatorAddress: String): Future[Boolean] = (validatorActorRegion ? ValidatorExists(uniqueId, operatorAddress)).mapTo[Boolean]

    def exists(operatorAddress: String): Future[Boolean] = checkExistsByOperatorAddress(operatorAddress)

    def jailValidatorWithActor(operatorAddress: String): Future[Int] = (validatorActorRegion ? JailValidator(uniqueId, operatorAddress)).mapTo[Int]

    def jailValidator(operatorAddress: String): Future[Int] = updateJailedStatus(operatorAddress = operatorAddress, jailed = true)

    def deleteValidator(operatorAddress: String): Future[Unit] = (validatorActorRegion ? DeleteValidatorWithOpAddress(uniqueId, operatorAddress)).mapTo[Unit]
    ...
    def getTotalVotingPowerWithActor: Future[MicroNumber] = (validatorActorRegion ? GetTotalVotingPower(uniqueId)).mapTo[MicroNumber]

...
class Tokens @Inject()(
  ...
  private def getTotalBondedTokenAmount: Future[String] = db.run(tokenTable.filter(_.denom === constants.Blockchain.StakingDenom).map(_.bondedAmount).result.head.asTry).map {
    case Success(result) => result
    case Failure(exception) => exception match {
      case noSuchElementException: NoSuchElementException => throw new BaseException(constants.Response.CRYPTO_TOKEN_NOT_FOUND, noSuchElementException)
    }
  ...
  object Utility {
    ...
    def onSlashing: Future[Unit] = {
      ...
      def updateStakingToken(stakingPoolResponse: StakingPoolResponse) = Service.updateStakingAmounts(denom = constants.Blockchain.StakingDenom, bondedAmount = stakingPoolResponse.result.bonded_tokens, notBondedAmount = stakingPoolResponse.result.not_bonded_tokens)
    ...
    def updateAll(): Future[Unit] = {
      ...
      def update(totalSupplyResponse: TotalSupplyResponse, mintingInflationResponse: MintingInflationResponse, stakingPoolResponse: StakingPoolResponse, communityPoolResponse: CommunityPoolResponse) = Future.traverse(totalSupplyResponse.result) { token =>
        Service.insertOrUpdate(Token(denom = token.denom, totalSupply = token.amount,
          bondedAmount = if (token.denom == constants.Blockchain.StakingDenom) stakingPoolResponse.result.bonded_tokens else MicroNumber.zero,
          notBondedAmount = if (token.denom == constants.Blockchain.StakingDenom) stakingPoolResponse.result.not_bonded_tokens else MicroNumber.zero,
          communityPool = communityPoolResponse.result.find(_.denom == token.denom).fold(MicroNumber.zero)(_.amount),
          inflation = if (token.denom == constants.Blockchain.StakingDenom) BigDecimal(mintingInflationResponse.result) else BigDecimal(0.0)
        ))
      }

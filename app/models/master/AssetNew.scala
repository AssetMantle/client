...
case class AssetNew(id: String, label: Option[String] = None, status: Option[Boolean], createdBy: Option[String] = None, createdOn: Option[Timestamp] = None, createdOnTimeZone: Option[String] = None, updatedBy: Option[String] = None, updatedOn: Option[Timestamp] = None, updatedOnTimeZone: Option[String] = None) extends Logged
...
class AssetsNew @Inject()(
  ...
  private def add(asset: AssetNew): Future[String] = db.run((assetTable returning assetTable.map(_.id) += asset).asTry).map {
  ...
  private def addMultiple(assets: Seq[AssetNew]): Future[Seq[String]] = db.run((assetTable returning assetTable.map(_.id) ++= assets).asTry).map {
    case Success(result) => result
    case Failure(exception) => exception match {
      case psqlException: PSQLException => throw new BaseException(constants.Response.PSQL_EXCEPTION, psqlException)
    }
  ...
  private def upsert(asset: AssetNew): Future[Int] = db.run(assetTable.insertOrUpdate(asset).asTry).map {
    case Success(result) => result
    case Failure(exception) => exception match {
      case psqlException: PSQLException => throw new BaseException(constants.Response.PSQL_EXCEPTION, psqlException)
    }
  ...
  private[models] class AssetTable(tag: Tag) extends Table[AssetNew](tag, "Asset") {

    def * = (id, label.?, status.?, createdBy.?, createdOn.?, createdOnTimeZone.?, updatedBy.?, updatedOn.?, updatedOnTimeZone.?) <> (AssetNew.tupled, AssetNew.unapply)
  ...
  object Service {
    ...
    def getAllByIDs(ids: Seq[String]): Future[Seq[AssetNew]] = getAllByAssetIDs(ids)

    def get(id: String): Future[Option[AssetNew]] = getByID(id)

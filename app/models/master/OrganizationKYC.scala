...
class OrganizationKYCs @Inject()(protected val databaseConfigProvider: DatabaseConfigProvider)(implicit executionContext: ExecutionContext) {
  ...
  private def getAllDocumentTypesByIDAndDocumentSet(id: String, documentTypes: Seq[String]): Future[Seq[String]] = db.run(organizationKYCTable.filter(_.id === id).filter(_.id.inSet(documentTypes)).map(_.documentType).result)

  private def getAllDocumentTypesByIDStatusAndDocumentSet(id: String, documentTypes: Seq[String], status: Boolean): Future[Seq[String]] = db.run(organizationKYCTable.filter(_.id === id).filter(_.id.inSet(documentTypes)).filter(_.status === status).map(_.documentType).result)
  ...
  object Service {
    ...
    def checkAllKYCFileTypesExists(id: String): Boolean = if (constants.File.ORGANIZATION_KYC_DOCUMENT_TYPES.diff(Await.result(getAllDocumentTypesByIDAndDocumentSet(id = id, documentTypes = constants.File.ORGANIZATION_KYC_DOCUMENT_TYPES), Duration.Inf)).isEmpty) false else true

    def checkAllKYCFilesVerified(id: String): Boolean = if (constants.File.ORGANIZATION_KYC_DOCUMENT_TYPES.diff(Await.result(getAllDocumentTypesByIDStatusAndDocumentSet(id = id, documentTypes = constants.File.ORGANIZATION_KYC_DOCUMENT_TYPES, status = true), Duration.Inf)).isEmpty) false else true

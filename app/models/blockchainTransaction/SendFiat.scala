...
case class SendFiat(from: String, to: String, amount: MicroInt, pegHash: String, gas: Int, status: Option[Boolean] = None, txHash: Option[String] = None, ticketID: String, mode: String, code: Option[String] = None, createdBy: Option[String] = None, createdOn: Option[Timestamp] = None, createdOnTimeZone: Option[String] = None, updatedBy: Option[String] = None, updatedOn: Option[Timestamp] = None, updatedOnTimeZone: Option[String] = None) extends BaseTransaction[SendFiat] with Logged {
...
class SendFiats @Inject()(
  ...
  case class SendFiatSerialized(from: String, to: String, amount: Long, pegHash: String, gas: Int, status: Option[Boolean], txHash: Option[String], ticketID: String, mode: String, code: Option[String], createdBy: Option[String], createdOn: Option[Timestamp], createdOnTimeZone: Option[String], updatedBy: Option[String], updatedOn: Option[Timestamp], updatedOnTimeZone: Option[String]) {
    def deserialize(): SendFiat = SendFiat(from = from, to = to, amount = new MicroInt(amount), pegHash = pegHash, gas = gas, status = status, txHash = txHash, ticketID = ticketID, mode = mode, code = code, createdBy = createdBy, createdOn = createdOn, createdOnTimeZone = createdOnTimeZone, updatedBy = updatedBy, updatedOn = updatedOn, updatedOnTimeZone = updatedOnTimeZone)
  }
    ...
    def onSuccess(ticketID: String, blockResponse: BlockResponse): Future[Unit] = {
      ...
      def updateBCFiat(bcFiatsInOrder: Seq[Fiat], negotiationID: String, orderResponse: OrderResponse.Response): Future[Unit] = orderResponse.value.fiat_peg_wallet match {
        case Some(fiatPegWallet) => {
          val updateFiats = Future.traverse(bcFiatsInOrder.map(_.pegHash).intersect(fiatPegWallet.map(_.pegHash)).flatMap(pegHash => fiatPegWallet.find(_.pegHash == pegHash)))(fiatPeg => {
            blockchainFiats.Service.update(Fiat(pegHash = fiatPeg.pegHash, ownerAddress = negotiationID, transactionID = fiatPeg.transactionID, transactionAmount =new MicroInt(fiatPeg.transactionAmount.toLong), redeemedAmount =new MicroInt(fiatPeg.redeemedAmount.toLong), dirtyBit = false))
          })
          val insertFiats = Future.traverse(fiatPegWallet.map(_.pegHash).diff(bcFiatsInOrder.map(_.pegHash)).flatMap(pegHash => fiatPegWallet.find(_.pegHash == pegHash)))(fiatPeg => {
            blockchainFiats.Service.create(pegHash = fiatPeg.pegHash, ownerAddress = negotiationID, transactionID = fiatPeg.transactionID, transactionAmount = new MicroInt(fiatPeg.transactionAmount.toLong), redeemedAmount =new MicroInt(fiatPeg.redeemedAmount.toLong), dirtyBit = false)
          })
          for {
            _ <- updateFiats
            _ <- insertFiats
      ...
      def createOrder(orderExists: Boolean, negotiationID: String, negotiation: masterNegotiation, amountSent: MicroInt): Future[Unit] = {
        ...
        if (!orderExists) {
          ...
          def masterOrderUpdate(fiatsInOrder: MicroInt, assetSent: String): Future[Int] = masterOrders.Service.update(masterOrder(id = negotiation.id, orderID = negotiationID, status = status(fiatsInOrder, assetSent == constants.Status.Asset.IN_ORDER)))

...
class SendAssets @Inject()(
  ...
  object Service {
    ...
    def getTransactionStatus(from: String, to: String, pegHash: String) = getTransactionByFromToAddressesAndPegHash(from, to, pegHash).map(x => if (x.isDefined) x.get.status else Option(false))
  }

...
@Singleton
class SendAssets @Inject()(protected val databaseConfigProvider: DatabaseConfigProvider, blockchainAssets: blockchain.Assets, blockchainOrders: blockchain.Orders, blockchainNegotiations: blockchain.Negotiations, transactionSendAsset: transactions.SendAsset, actorSystem: ActorSystem, implicit val pushNotifications: PushNotifications, implicit val masterAccounts: master.Accounts, implicit val blockchainAccounts: blockchain.Accounts)(implicit wsClient: WSClient, configuration: Configuration, executionContext: ExecutionContext)  {
  ...
  object Utility {
    ...
    def onFailure(ticketID: String, message: String): Future[Unit] = Future {
      try {
        ...
        val sendAsset = Service.getTransaction(ticketID)
        pushNotifications.sendNotification(masterAccounts.Service.getId(sendAsset.to), constants.Notification.FAILURE, Seq(message))
        pushNotifications.sendNotification(sendAsset.from, constants.Notification.FAILURE, Seq(message))
      } catch {

...
class OrderCancels @Inject()(
  ...
  def serialize(orderCancel: OrderCancel): OrderCancelSerialized = OrderCancelSerialized(from = orderCancel.from, fromID = orderCancel.fromID, orderID = orderCancel.orderID, gas = orderCancel.gas.toMicroString, status = orderCancel.status, txHash = orderCancel.txHash, ticketID = orderCancel.ticketID, mode = orderCancel.mode, code = orderCancel.code, createdBy = orderCancel.createdBy, createdOn = orderCancel.createdOn, createdOnTimeZone = orderCancel.createdOnTimeZone, updatedBy = orderCancel.updatedBy, updatedOn = orderCancel.updatedOn, updatedOnTimeZone = orderCancel.updatedOnTimeZone)
  ...
  private implicit val module: String = constants.Module.BLOCKCHAIN_TRANSACTION_ORDER_CANCEL

  private val schedulerExecutionContext: ExecutionContext = actors.Service.actorSystem.dispatchers.lookup("akka.actor.scheduler-dispatcher")

  import databaseConfig.profile.api._

  private[models] val orderCancelTable = TableQuery[OrderCancelTable]
  ...
  private def add(orderCancel: OrderCancel): Future[String] = db.run((orderCancelTable returning orderCancelTable.map(_.ticketID) += serialize(orderCancel)).asTry).map {
  ...
  private def updateStatusAndCodeOnTicketID(ticketID: String, status: Option[Boolean], code: String): Future[Int] = db.run(orderCancelTable.filter(_.ticketID === ticketID).map(x => (x.status.?, x.code)).update((status, code)).asTry).map {
  ...
  private def updateTxHashAndStatusOnTicketID(ticketID: String, txHash: Option[String], status: Option[Boolean]): Future[Int] = db.run(orderCancelTable.filter(_.ticketID === ticketID).map(x => (x.txHash.?, x.status.?)).update((txHash, status)).asTry).map {
    case Success(result) => result match {
      case 0 => throw new BaseException(constants.Response.NO_SUCH_ELEMENT_EXCEPTION)
      case _ => result
    }
  ...
  private def updateStatusByTicketID(ticketID: String, status: Option[Boolean]): Future[Int] = db.run(orderCancelTable.filter(_.ticketID === ticketID).map(_.status.?).update(status).asTry).map {
    case Success(result) => result match {
      case 0 => throw new BaseException(constants.Response.NO_SUCH_ELEMENT_EXCEPTION)
      case _ => result
    }
  ...
  private def findByTicketID(ticketID: String): Future[OrderCancelSerialized] = db.run(orderCancelTable.filter(_.ticketID === ticketID).result.head.asTry).map {
    case Success(result) => result
    case Failure(exception) => exception match {
  ...
  private def findTransactionHashByTicketID(ticketID: String): Future[Option[String]] = db.run(orderCancelTable.filter(_.ticketID === ticketID).map(_.txHash.?).result.head.asTry).map {
    case Success(result) => result
    case Failure(exception) => exception match {
      case noSuchElementException: NoSuchElementException => throw new BaseException(constants.Response.NO_SUCH_ELEMENT_EXCEPTION, noSuchElementException)
    }
  ...
  private def findModeByTicketID(ticketID: String): Future[String] = db.run(orderCancelTable.filter(_.ticketID === ticketID).map(_.mode).result.head.asTry).map {
    case Success(result) => result
    case Failure(exception) => exception match {
      case noSuchElementException: NoSuchElementException => throw new BaseException(constants.Response.NO_SUCH_ELEMENT_EXCEPTION, noSuchElementException)
    }
  ...
  private def getTicketIDsWithNullStatus: Future[Seq[String]] = db.run(orderCancelTable.filter(_.status.?.isEmpty).map(_.ticketID).result)

  private def updateTxHashOnTicketID(ticketID: String, txHash: Option[String]): Future[Int] = db.run(orderCancelTable.filter(_.ticketID === ticketID).map(x => x.txHash.?).update(txHash).asTry).map {
    case Success(result) => result match {
      case 0 => throw new BaseException(constants.Response.NO_SUCH_ELEMENT_EXCEPTION)
      case _ => result
    }
  ...
  object Service {
    ...
    def create(orderCancel: OrderCancel): Future[String] = add(orderCancel)
    ...
    def getTransaction(ticketID: String): Future[OrderCancel] = findByTicketID(ticketID).map(_.deserialize)

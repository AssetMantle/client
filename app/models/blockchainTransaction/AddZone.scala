  ...
  object Utility {
    def onSuccess(ticketID: String, response: Response): Future[Unit] = Future {
      try {
        ...
        pushNotification.sendNotification(masterAccounts.Service.getId(addZone.to), constants.Notification.SUCCESS, Seq(response.TxHash))
        pushNotification.sendNotification(addZone.from, constants.Notification.SUCCESS, Seq(response.TxHash))
      } catch {
    ...
    def onFailure(ticketID: String, message: String): Future[Unit] = Future {
      try {
        ...
        val addZone = Service.getAddZone(ticketID)
        pushNotification.sendNotification(masterAccounts.Service.getId(addZone.to), constants.Notification.FAILURE, Seq(message))
        pushNotification.sendNotification(addZone.from, constants.Notification.FAILURE, Seq(message))
      } catch {
        case baseException: BaseException => logger.error(constants.Error.BASE_EXCEPTION, baseException)
      }
    }

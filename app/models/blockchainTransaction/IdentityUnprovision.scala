...
case class IdentityUnprovision(from: String, to: String, identityID: String, gas: MicroNumber, status: Option[Boolean] = None, txHash: Option[String] = None, ticketID: String, mode: String, code: Option[String] = None, createdBy: Option[String] = None, createdOn: Option[Timestamp] = None, createdOnTimeZone: Option[String] = None, updatedBy: Option[String] = None, updatedOn: Option[Timestamp] = None, updatedOnTimeZone: Option[String] = None) extends BaseTransaction[IdentityUnprovision] with Logged {
  def mutateTicketID(newTicketID: String): IdentityUnprovision = IdentityUnprovision(from = from, to = to, identityID = identityID, gas = gas, status = status, txHash = txHash, ticketID = newTicketID, mode = mode, code = code)
}
...
class IdentityUnprovisions @Inject()(
  ...
  case class IdentityUnprovisionSerialized(from: String, to: String, identityID: String, gas: String, status: Option[Boolean], txHash: Option[String], ticketID: String, mode: String, code: Option[String], createdBy: Option[String], createdOn: Option[Timestamp], createdOnTimeZone: Option[String], updatedBy: Option[String], updatedOn: Option[Timestamp], updatedOnTimeZone: Option[String]) {
    def deserialize: IdentityUnprovision = IdentityUnprovision(from = from, to = to, identityID = identityID, gas = new MicroNumber(BigInt(gas)), status = status, txHash = txHash, ticketID = ticketID, mode = mode, code = code, createdBy = createdBy, createdOn = createdOn, createdOnTimeZone = createdOnTimeZone, updatedOn = updatedOn, updatedBy = updatedBy, updatedOnTimeZone = updatedOnTimeZone)
  }

  def serialize(identityUnprovision: IdentityUnprovision): IdentityUnprovisionSerialized = IdentityUnprovisionSerialized(from = identityUnprovision.from, to = identityUnprovision.to, identityID = identityUnprovision.identityID, gas = identityUnprovision.gas.toMicroString, status = identityUnprovision.status, txHash = identityUnprovision.txHash, ticketID = identityUnprovision.ticketID, mode = identityUnprovision.mode, code = identityUnprovision.code, createdBy = identityUnprovision.createdBy, createdOn = identityUnprovision.createdOn, createdOnTimeZone = identityUnprovision.createdOnTimeZone, updatedBy = identityUnprovision.updatedBy, updatedOn = identityUnprovision.updatedOn, updatedOnTimeZone = identityUnprovision.updatedOnTimeZone)
  ...
  private implicit val module: String = constants.Module.BLOCKCHAIN_TRANSACTION_IDENTITY_UNPROVISION
  ...
  private def add(identityUnprovision: IdentityUnprovision): Future[String] = db.run((identityUnprovisionTable returning identityUnprovisionTable.map(_.ticketID) += serialize(identityUnprovision)).asTry).map {
  ...
  private def findByTicketID(ticketID: String): Future[IdentityUnprovisionSerialized] = db.run(identityUnprovisionTable.filter(_.ticketID === ticketID).result.head.asTry).map {
    case Success(result) => result
    case Failure(exception) => exception match {
      case noSuchElementException: NoSuchElementException => throw new BaseException(constants.Response.NO_SUCH_ELEMENT_EXCEPTION, noSuchElementException)
    }
  ...
  private def findTransactionHashByTicketID(ticketID: String): Future[Option[String]] = db.run(identityUnprovisionTable.filter(_.ticketID === ticketID).map(_.txHash.?).result.head.asTry).map {
    case Success(result) => result
    case Failure(exception) => exception match {
      case noSuchElementException: NoSuchElementException => throw new BaseException(constants.Response.NO_SUCH_ELEMENT_EXCEPTION, noSuchElementException)
    }
  ...
  private def findModeByTicketID(ticketID: String): Future[String] = db.run(identityUnprovisionTable.filter(_.ticketID === ticketID).map(_.mode).result.head.asTry).map {
    case Success(result) => result
    case Failure(exception) => exception match {
      case noSuchElementException: NoSuchElementException => throw new BaseException(constants.Response.NO_SUCH_ELEMENT_EXCEPTION, noSuchElementException)
    }
  ...
  object Service {
    ...
    def create(identityUnprovision: IdentityUnprovision): Future[String] = add(identityUnprovision)
    ...
    def getTransaction(ticketID: String): Future[IdentityUnprovision] = findByTicketID(ticketID).map(_.deserialize)

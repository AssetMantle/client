...
case class ReceiveFiat(id: String, traderID: String, orderID: String, amount: MicroInt, status: String, createdBy: Option[String] = None, createdOn: Option[Timestamp] = None, createdOnTimeZone: Option[String] = None, updatedBy: Option[String] = None, updatedOn: Option[Timestamp] = None, updatedOnTimeZone: Option[String] = None) extends Logged
...
class ReceiveFiats @Inject()(protected val databaseConfigProvider: DatabaseConfigProvider)(implicit executionContext: ExecutionContext) {
  ...
  case class ReceiveFiatSerialized(id: String, traderID: String, orderID: String, amount: Long, status: String, createdBy: Option[String], createdOn: Option[Timestamp], createdOnTimeZone: Option[String], updatedBy: Option[String], updatedOn: Option[Timestamp], updatedOnTimeZone: Option[String]) {
    def deserialize(): ReceiveFiat = ReceiveFiat(id = id, traderID = traderID, orderID = orderID, amount = new MicroInt(amount), status = status, createdBy = createdBy, createdOn = createdOn, createdOnTimeZone = createdOnTimeZone, updatedBy = updatedBy, updatedOn = updatedOn, updatedOnTimeZone = updatedOnTimeZone)
  }
  ...
  private def getByTraderIDsAndStatuses(traderIDs: Seq[String], status: Seq[String]): Future[Seq[ReceiveFiatSerialized]] = db.run(receiveFiatTable.filter(_.traderID inSet traderIDs).filter(_.status inSet status).result)

  private def getByTraderIDAndStatuses(traderID: String, status: Seq[String]): Future[Seq[ReceiveFiatSerialized]] = db.run(receiveFiatTable.filter(_.traderID === traderID).filter(_.status inSet status).result)
  ...
  object Service {
    def create(traderID: String, orderID: String, amount: MicroInt, status: String): Future[String] = add(serialize(ReceiveFiat(utilities.IDGenerator.requestID(), traderID, orderID, amount, status)))

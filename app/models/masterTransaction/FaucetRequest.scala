...
case class FaucetRequest(id: String, ticketID: Option[String] = None, accountID: String, amount: MicroLong, gas: Option[MicroLong] = None, status: Option[Boolean] = None, comment: Option[String] = None, createdBy: Option[String] = None, createdOn: Option[Timestamp] = None, createdOnTimeZone: Option[String] = None, updatedBy: Option[String] = None, updatedOn: Option[Timestamp] = None, updatedOnTimeZone: Option[String] = None) extends Logged
...
class FaucetRequests @Inject()(protected val databaseConfigProvider: DatabaseConfigProvider)(implicit executionContext: ExecutionContext) {
  ...
  def serialize(faucetRequest: FaucetRequest): FaucetRequestSerialized = FaucetRequestSerialized(id = faucetRequest.id, ticketID = faucetRequest.ticketID, accountID = faucetRequest.accountID, amount = faucetRequest.amount.toMicroLong, gas = faucetRequest.gas.flatMap(x => Option(x.toMicroLong)), status = faucetRequest.status, comment = faucetRequest.comment, createdBy = faucetRequest.createdBy, createdOn = faucetRequest.createdOn, createdOnTimeZone = faucetRequest.createdOnTimeZone, updatedBy = faucetRequest.updatedBy, updatedOn = faucetRequest.updatedOn, updatedOnTimeZone = faucetRequest.updatedOnTimeZone)

  case class FaucetRequestSerialized(id: String, ticketID: Option[String], accountID: String, amount: Long, gas: Option[Long], status: Option[Boolean], comment: Option[String], createdBy: Option[String], createdOn: Option[Timestamp], createdOnTimeZone: Option[String], updatedBy: Option[String], updatedOn: Option[Timestamp], updatedOnTimeZone: Option[String]) {
    def deserialize: FaucetRequest = FaucetRequest(id = id, ticketID = ticketID, accountID = accountID, amount = new MicroLong(amount), gas = if (gas.isDefined) Option(new MicroLong(gas.get)) else None, status = status, comment = comment, createdBy = createdBy, createdOn = createdOn, createdOnTimeZone = createdOnTimeZone, updatedBy = updatedBy, updatedOn = updatedOn, updatedOnTimeZone = updatedOnTimeZone)
  }
  ...
  private def updateTicketIDGasAndStatusByID(id: String, ticketID: String, gas: Option[MicroLong], status: Option[Boolean]): Future[Int] = db.run(faucetRequestTable.filter(_.id === id).map(faucet => (faucet.ticketID, faucet.gas.?, faucet.status.?)).update(ticketID, gas.flatMap(x => Option(x.toMicroLong)), status).asTry).map {
    case Success(result) => result
    case Failure(exception) => exception match {
      case psqlException: PSQLException => throw new BaseException(constants.Response.PSQL_EXCEPTION, psqlException)
      case noSuchElementException: NoSuchElementException => throw new BaseException(constants.Response.NO_SUCH_ELEMENT_EXCEPTION, noSuchElementException)
  ...
  object Service {
    ...
    def create(accountID: String, amount: MicroLong): Future[String] = add(FaucetRequest(id = utilities.IDGenerator.requestID(), accountID = accountID, amount = amount))

    def accept(requestID: String, gas: MicroLong, ticketID: String): Future[Int] = updateTicketIDGasAndStatusByID(id = requestID, ticketID = ticketID, gas = Option(gas), status = Option(true))

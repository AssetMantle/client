...
class Block @Inject()(
  ...
  def insertTransactionsOnBlock(header: Header): Future[Seq[blockchainTransaction]] = {
    ...
    def insertTransactions(transactionsHash: Seq[String]): Future[Seq[blockchainTransaction]] = if (transactionsHash.nonEmpty) {
      ...
      def updateMessageCounter(transactions: Seq[blockchainTransaction]) = analyticMessageCounters.Utility.updateMessageCounter(transactions.flatMap(_.getMessageCounters).groupBy(_._1).view.mapValues(_.map(_._2).toList.sum).toMap)

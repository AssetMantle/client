...
class Block @Inject()(
  ...
  private def actionsOnTransactionsMessages(transactions: Seq[blockchainTransaction], height: Int) = Future.traverse(transactions) { transaction =>
    if (transaction.status) {
      Future.traverse(transaction.messages) { stdMsg =>
        try {

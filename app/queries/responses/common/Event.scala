...
case class Event(`type`: String, attributes: Seq[Attribute]) {
  ...
  def decode: Event = Event(`type` = `type`, attributes = attributes.map(x => Attribute(key = utilities.Hash.base64URLDecoder(x.key), value = x.value.fold[Option[String]](None)(y => Option(utilities.Hash.base64URLDecoder(y))))))
}

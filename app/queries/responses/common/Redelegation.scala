...
object Redelegation {
  ...
  case class RedelegationEntry(creation_height: Int, completion_time: RFC3339, initial_balance: MicroNumber, shares_dst: String) {
    def toRedelegationEntry: Serializable.RedelegationEntry = Serializable.RedelegationEntry(creationHeight = creation_height, completionTime = completion_time, initialBalance = initial_balance, sharesDestination = BigDecimal(shares_dst))
  }

  def redelegationEntryApply(creation_height: Int, completion_time: String, initial_balance: String, shares_dst: String): RedelegationEntry = RedelegationEntry(creation_height = creation_height, completion_time = RFC3339(completion_time), initial_balance = new MicroNumber(BigDecimal(initial_balance).toBigInt), shares_dst = shares_dst)

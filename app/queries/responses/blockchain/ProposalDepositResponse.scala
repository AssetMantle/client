...
object ProposalDepositResponse {
  ...
  case class Deposit(proposal_id: String, depositor: String, amount: Seq[Coin]) {
    def toSerializableProposalDeposit: BlockchainProposalDeposit = BlockchainProposalDeposit(proposalID = proposal_id, depositor = depositor, amount = amount.map(_.toCoin))
  }

...
import queries.responses.memberCheck.MemberScanResultResponse.{Response, ResponsePart1, ResponsePart2}
import queries.responses.memberCheck.MemberScanResultResponse
...
@Singleton
class GetMemberScanResult @Inject()(wsClient: WSClient, keyStore: KeyStore)(implicit configuration: Configuration, executionContext: ExecutionContext) {
  ...
  private def action(request: String): Future[Response] = {
    ...
    } yield Response(responsePart1.id, MemberScanResultResponse.entity(responsePart1.person, responsePart2.person))
  }

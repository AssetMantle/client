...
class ChangePassword @Inject()(wsClient: WSClient)(implicit configuration: Configuration, executionContext: ExecutionContext) {
  ...
  case class Request(oldPassword: String, newPassword: String, confirmNewPassword: String) extends BaseRequestEntity
  ...
  private def action(username: String, request: Request): Future[Response] = wsClient.url(url + username).put(Json.toJson(request)).map { response => utilities.JSON.getResponseFromJson[Response](response) }

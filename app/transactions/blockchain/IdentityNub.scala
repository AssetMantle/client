...
@Singleton
class IdentityNub @Inject()(wsClient: WSClient)(implicit configuration: Configuration, executionContext: ExecutionContext) {

  private implicit val module: String = constants.Module.TRANSACTIONS_IDENTITY_NUB

  private implicit val logger: Logger = Logger(this.getClass)

  private val path = "xprt/identities/nub"
  ...
  case class Message(baseReq: BaseReq, nubID: String)
  ...
  private implicit val requestWrites: Writes[Request] = (request: Request) => Json.obj(
    "type" -> constants.Blockchain.TransactionRequest.IDENTITY_NUB,

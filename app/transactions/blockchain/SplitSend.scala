...
@Singleton
class SplitSend @Inject()(wsClient: WSClient)(implicit configuration: Configuration, executionContext: ExecutionContext) {

  private implicit val module: String = constants.Module.TRANSACTIONS_SPLIT_SEND

  private implicit val logger: Logger = Logger(this.getClass)

  private val path = "xprt/splits/send"
  ...
  case class Message(baseReq: BaseReq, fromID: String, toID: String, ownableID: String, split: BigDecimal)

  private implicit val messageWrites: OWrites[Message] = (message: Message) => Json.obj(
    "baseReq" -> Json.toJson(message.baseReq),
    "fromID" -> message.fromID,
    "toID" -> message.toID,
    "ownableID" -> message.ownableID,
    "split" -> message.split.bigDecimal.toPlainString,
  )
  ...
  private implicit val requestWrites: Writes[Request] = (request: Request) => Json.obj(
    "type" -> constants.Blockchain.TransactionRequest.SPLIT_SEND,

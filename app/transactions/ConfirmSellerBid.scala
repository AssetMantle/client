...
class ConfirmSellerBid @Inject()(wsClient: WSClient)(implicit configuration: Configuration, executionContext: ExecutionContext) {
  ...
  case class Request(base_req: BaseReq, to: String, bid: MicroNumber, time: String, pegHash: String, sellerContractHash: String, mode: String, password: String) extends BaseRequest

  object Request {

    def apply(base_req: BaseReq, to: String, bid: String, time: String, pegHash: String, sellerContractHash: String, mode: String, password: String): Request = new Request(base_req, to, new MicroNumber(BigInt(bid)), time, pegHash, sellerContractHash, mode, password)

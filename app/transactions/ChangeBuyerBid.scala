...
class ChangeBuyerBid @Inject()(wsClient: WSClient)(implicit configuration: Configuration, executionContext: ExecutionContext) {
  ...
  case class Request(base_req: BaseReq, to: String, bid: MicroLong, time: String, pegHash: String, mode: String, password: String) extends BaseRequest

  object Request {

    def apply(base_req: BaseReq, to: String, bid: String, time: String, pegHash: String, mode: String, password: String): Request = new Request(base_req, to, new MicroLong(bid.toLong), time, pegHash, mode, password)
